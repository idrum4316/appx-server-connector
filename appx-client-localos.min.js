...
appxIsLocalReady()?(console.log("Setting Clipboard Data:  "+a.data),ms={cmd:"setclipboard",args:[a.data],handler:"localos_setclipboard_handler",data:null,authToken:localStorage.authToken},localos_session.ws.send(JSON.stringify(ms))):createClipboardDialog(!0,e.data.data)}function appxgetclipboardhandler(e){appxIsLocalReady()?(ms={cmd:"getclipboard",args:["getclipboard"],handler:"localos_exeoscmd_handler",data:null,authToken:localStorage.authToken},localos_session.ws.send(JSON.stringify(ms))):createClipboardDialog(!1)}function buildOpenCommand(e){var a=""
...
return e.indexOf("/H")>-1&&(e=e.replace(/\/h /i,"/b ")),e.indexOf("/W")>-1&&(e=e.replace(/\/w /i,"/WAIT ")),a+=appx_session.globals.os.indexOf("Win")>-1?'start "" '+e:appx_session.globals.os.indexOf("Mac")>-1?"open "+e:"xdg-open "+e}function appxloadurlhandler(e){var a=e.data,s={},n=a.trim(),o=!1,i=null
try{var l=n.toLowerCase().trim().length
if((n.toLowerCase().substring(0,2).indexOf("/w")>-1||n.toLowerCase().substring(l-2,l).indexOf("/w")>-1)&&(n=a.replace("/w","").trim(),o=!0),appx_session.globals.os.indexOf("Win")&&n.substring(0,1).indexOf("@")>-1&&(n=n),n.substring(0,1).indexOf("@")>-1)try{s={cmd:"execoscmd",args:[n.substring(1)],handler:"localos_exeoscmd_handler",data:null,authToken:localStorage.authToken},appxIsLocalReady()?localos_session.ws.send(JSON.stringify(s)):alert(appx_session.language.alerts.localCommandOSError+n)}catch(t){console.log("CharView.loadUrl("+n.substring(1)+"): Run Failed"),console.log("Run Failure Exception = "+t),console.log(t.stack)}else if(n.indexOf("$print:")>-1)try{var p=appx_session.parseOption(n.substring(7).trim())
s=appx_session.globals.os.indexOf("Win")>-1?{cmd:"execoscmd",args:['"'+appx_session.local_environment.currentworkingdirectory+'\\bin\\winprint.exe" '+p],handler:"localos_exeoscmd_handler",data:null,authToken:localStorage.authToken}:{cmd:"execoscmd",args:['"'+appx_session.local_environment.currentworkingdirectory+'/bin/appx_print" '+p],handler:"localos_exeoscmd_handler",data:null,authToken:localStorage.authToken},appxIsLocalReady()?localos_session.ws.send(JSON.stringify(s)):alert(appx_session.language.alerts.localPrintOSError+n)}catch(t){console.log("CharView.loadUrl("+n.substring(1)+"): Run Failed"),console.log("Run Failure Exception = "+t),console.log(t.stack)}else if(n.indexOf("$printSetup:")>-1)console.log("FIXME - newInstance() called"),s={cmd:"execoscmd",args:[appx_session.parseOption("$(cacheRoot)")+"\\print-dialog.exe"],handler:"localos_exeoscmd_handler",data:null,authToken:localStorage.authToken},appxIsLocalReady()?localos_session.ws.send(JSON.stringify(s)):alert(appx_session.language.alerts.localOSError)
else if(n.indexOf("$display:")>-1)s={cmd:"execoscmd",args:[buildOpenCommand(appx_session.parseOption(n.substring(9)))],handler:"localos_exeoscmd_handler",data:null,authToken:localStorage.authToken},appxIsLocalReady()?localos_session.ws.send(JSON.stringify(s)):alert(appx_session.language.alerts.localDisplayOSError+n)
else if(n.indexOf("$beep:")>-1){var r=$('<embed height="50" width="100" src="'+appxClientRoot+'/assets/beep.wav">').appendTo("body").hide()
setTimeout(function(){$(r).remove()},1500)}else if(n.indexOf("$messagebeep:")>-1){var r=$('<embed height="50" width="100" src="'+appxClientRoot+'/assets/message.wav">').appendTo("body").hide()
setTimeout(function(){$(r).remove()},1500)}else if(n.indexOf("$warningbeep:")>-1){var r=$('<embed height="50" width="100" src="'+appxClientRoot+'/assets/warning.wav">').appendTo("body").hide()
setTimeout(function(){$(r).remove()},1500)}else if(n.indexOf("$cancelbeep:")>-1){var r=$('<embed height="50" width="100" src="'+appxClientRoot+'/assets/cancel.wav">').appendTo("body").hide()
setTimeout(function(){$(r).remove()},1500)}else if(n.indexOf("$errorbeep:")>-1){var r=$('<embed height="50" width="100" src="'+appxClientRoot+'/assets/error.wav">').appendTo("body").hide()
setTimeout(function(){$(r).remove()},1500)}else if(n.indexOf("$play:")>-1){var d=$("<audio>")
...
appx_session.setPropEx(c[0].trim(),c[1],!1)}else if(n.indexOf("$newsession:")>-1){for(var g=n.substring(12,n.length).trim().split(" "),u={},f=0;f<g.length;f++){var x=g[f].split("=")
""!=x[0]&&(u[x[0].replace("-","")]=x[1].replace(/"/g,""))}sendappxnewsession(appx_session.host,appx_session.port,appx_session.user,appx_session.password,appx_session.screenrows,appx_session.screencols,u)}else if(n.indexOf("$(pushAndOpen")>-1||n.indexOf("$(pushAndSave")>-1&&"Edge"!=checkBrowser()&&n.indexOf(".pdf")>-1){var h=appx_session.appxCacheUrl+"/getFile/"+n.replace("$(","").replace(")","")
h=encodeURI(h)
...
v[0]="",i&&(v[0]=i),v[1]=n
try{s={cmd:"execoscmd",args:[buildOpenCommandDos(appx_session.parseOption(v[1]))],handler:"localos_exeoscmd_handler",data:null,authToken:localStorage.authToken},appxIsLocalReady()?localos_session.ws.send(JSON.stringify(s)):(n.indexOf("http")<0&&(n="http://"+n),window.open(n,"_blank","toolbar=yes, scrollbars=yes, resizable=yes, location=yes, menubar=yes, status=yes, titlebar=yes, channelmode=yes, top=50, left=50, width="+.8*$(window).width()+", height="+.8*$(window).height()))}catch(t){console.log("CharView.loadUrl("+v[0]+" "+n+"): Run Failed"),console.log("Run Failure Exception = "+t),console.log(t.stack)}}}catch(t){console.log("appxloadurlhandler("+n+"): Bad URL, e="+t),console.log(t.stack)}}function appxreceivefilehandler(e){try{var a=e.data
...
appx_session.ws.send(JSON.stringify(s))}else if(a.indexOf("$(signature)")>-1){var n=$("<div>").addClass("appxsignaturedialog").css({"z-index":2e5}),o=$("<canvas>").addClass("signaturepad")
$(n).append(o),$("body").append(n),$.each($(".appxsignaturedialog"),function(e,a){$(a).dialog({open:function(e,a){$(this).parent().addClass("appx-signature-dialog-parent").position({my:"center",at:"center",of:"#box_0"}),$(this).addClass("appx-signature-dialog"),appx_session.signaturePad=new SignaturePad($(".signaturepad")[0]),setTimeout(function(){var e=Math.max(window.devicePixelRatio||1,1)
...
var s={cmd:"openfile",args:[a],mongoHost:appxConnectorHost,uid:appx_session.user,pid:appx_session.pid.trim(),port:appxConnectorMongoPort,httpPath:appxConnectorPathHttp,protocol:appxProtocol,handler:"localopenfile",data:null,authToken:localStorage.authToken}
...
appx_session.ws.send(JSON.stringify(e))}$(this).dialog("destroy").remove()}}),$(a).dialog("open")})}else $("#fileChooser").val(""),$(".appxfiledialog").dialog("open")}catch(i){console.log(i),console.log(i.stack)
var s={cmd:"appxmessage",args:[3,0],handler:"appxreceivefilehandler",data:null}
appx_session.ws.send(JSON.stringify(s))}}function appxReadBlob(e,a){var s="fileopener",n=!1,o=null
if(e&&(s=e),"DnD"===s?(o=appx_session.dropEvent.dataTransfer.files,appx_session.filesUploadArray=appx_session.dropEvent.dataTransfer.files):o=$("#fileChooser")[0].files,!o.length)return void alert(appx_session.language.alerts.fileError)
o.length>1&&(n=!0)
for(var i=0;i<o.length;i++){var l=o[i]
uploadFileToMongo(new Blob([l]),l.name,a)}}function CreateFile(e){var a={cmd:"createfile",args:[e],handler:"localfilecreate",data:null,authToken:localStorage.authToken}
...
var s={cmd:"appendfile",args:[a],handler:"localfileappend",data:null,authToken:localStorage.authToken}
...
appx_session.ws.send(JSON.stringify(s))}function fileUploadError(e){appxClearStatusMsgText(),alert('"'+e+appx_session.language.alerts.folderSelectedError),$("#main").unblock()}function createClipboardDialog(e,a){var s,n,o,i,l
e?(s=appx_session.language.tooltips.clipboardToText,n=appx_session.language.buttons.copyText,o="copyToClipboardDiv",i=appx_session.language.tooltips.clipboardTo,l="copyToClipboard"):(s=appx_session.language.tooltips.clipboardFromText,n=appx_session.language.buttons.submit,o="copyFromClipboardDiv",i=appx_session.language.tooltips.clipboardFrom,l="copyFromClipboard")
var t=$("<div>").attr("id",o),p=$('<label for="'+o+'">').addClass("clipboard").html(s),r=$('<textarea rows="10" cols="50" id="'+l+'">').addClass(o).val(a),d=$("<button>").attr("data-clipboard-target","#"+l).addClass("btn clipboard").html(n)
if(p.append("<br /><br /><br />").appendTo(t),r.appendTo(t),d.appendTo(t),t.dialog({title:i,position:{of:"#appx_main_container"},minWidth:700,height:300}),t.on("dialogclose",function(a){if(e)c.destroy()
else{var s=r.val(),n={cmd:"appxclipboard",args:[s],handler:"appxsendfilehandler",data:null}
appx_session.ws.send(JSON.stringify(n))}}),e){var c=new Clipboard(".btn")
c.on("success",function(e){e.clearSelection(),t.dialog("destroy").remove()})}else d.on("click",function(e){t.dialog("destroy").remove()})}function localos_append_file_handler(e){}function localConnectorUpdateCertificate(e){var a={cmd:"getCertificate",args:[0],handler:"getCertificate",data:e,authToken:localStorage.authToken}
localos_session.ws.send(JSON.stringify(a))}var msgCount=0
