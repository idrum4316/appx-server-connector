...
for(var a in CKEDITOR.instances){var s=$("#cke_"+a)
s.val(CKEDITOR.instances[a].getData()),CKEDITOR.instances[a].checkDirty()&&(s.addClass("appxitem dirty"),s.attr("id",a))}var o=[]
$("#appx_main_container").focus(),$(".dirty").each(function(){o.push(this)}),1==appx_session.processhelp&&(appx_session.processhelpoption=e,console.log("option: "+e),console.log("processhelpoption: "+appx_session.processhelpoption),e=OPT_HELP_OPT),sendappxshow(e,o)}}function appxwidgetdimensions(e,t,a){try{var s,o,i,l=0,n=0,r=0,p=0,w=e,d=appx_session.colWidthPx,c=appx_session.rowHeightPx
if(s=w.hasOwnProperty("widget")?w.widget.wWidgetType:w.wWidgetType,a&&(l=a.begin_column-1,n=a.begin_row,appxIsScrollReg(a)||n--),w.hasOwnProperty("wWidgetId")||(e.hasOwnProperty("widget")&&null!=e.widget&&null!=e.widget.wPositionX?(w=e.widget,s!=WIDGET_TYPE_PROGRESS_BAR&&s!=WIDGET_TYPE_SLIDER&&(w.wSizeW||(w.wSizeW=e.size_col||e.size_cols),w.wSizeH||(w.wSizeH=e.size_row||e.size_rows))):(l=w.pos_col*d,n=w.pos_row*c,r=(w.size_cols||w.size_col)*d,p=(w.size_rows||w.size_row)*c)),w.hasOwnProperty("wWidgetId")){var g=0,h=0,T=0,x=0
w.wOffsetX&&(g=~~(w.wOffsetX*d/100)),w.wOffsetY&&(h=~~(w.wOffsetY*c/100)),w.wOffsetW&&(T=~~(w.wOffsetW*d/100)),w.wOffsetH&&(x=~~(w.wOffsetH*c/100)),null!=w.wPositionX&&(l=w.wPositionX?(w.wPositionX-l)*d+g:0,n=w.wPositionY?(w.wPositionY-n)*c+h:0,r=w.wSizeW?w.wSizeW*d+T:0,p=w.wSizeH?w.wSizeH*c+x:0)}if(t){o=e.pos_row,i=e.pos_col
parseInt(t.css("top").substring(0,t.css("top").indexOf("p")))
if(a&&(appxIsScrollReg(a)||appxIsScroll(a))){var u=(o-a.begin_row)*appx_session.rowHeightPx,_=(i-a.begin_column)*appx_session.colWidthPx
n=u,l=_}if(t.css({position:"absolute",left:l,top:n}),s===WIDGET_TYPE_LINE||0!==p&&0!==r||t.hide(),s==WIDGET_TYPE_HTML_VIEWER&&(p-=2),t.css("height",p),s!=WIDGET_TYPE_CHECK_BOX){if(s!=WIDGET_TYPE_TABLE&&s!=WIDGET_TYPE_BOX&&s!=WIDGET_TYPE_BUTTON&&s!=WIDGET_TYPE_LABEL&&s!=WIDGET_TYPE_PICTURE&&s!=WIDGET_TYPE_LISTBOX&&(r+=5),s==WIDGET_TYPE_LISTBOX){var E=3.96*d
t.attr("data-padWidth",E),r+=E}t.hasClass("button")&&t.hasClass("error")&&(r=315),t.css("width",r)}s==WIDGET_TYPE_BOX&&(w.wLabel&&w.wLabel.length>0?t.css({width:r-2*d-2+14,height:p-c+5+14,left:l+2}):t.css({width:r-2*d+2+14,height:p-c+5+2+14})),t.hasClass("defaultBorder")&&t.is("button")&&w.wBorder!=BORDER_NONE&&t.css({top:dim.y+1,left:dim.x+1,height:dim.h-2,width:dim.w-2})}}catch(b){console.log("appxwidgetdimensions: "+b),console.log(b.stack)}return{x:l,y:n,w:r,h:p,cw:d,rh:c}}function appxwidgetshandler(e){for(var t=e.data,a=1,s=!1;t.length>0;){var o=t.shift(),i=new Widget(o[0],"",o[1],o[2],o[3]),l=parseInt(i.wWidgetType)
if(i.wDefaultButton&&(s?i.wDefaultButton=null:s=!0),l!=WIDGET_TYPE_ROWTEXT){var n=null
...
$tag.html("")}switch(wt){case WIDGET_TYPE_LINE:wx.wSizeH||(wx.wSizeH=wx.size_row),wx.wSizeW||(wx.wSizeW=wx.size_col)
...
$(this).addClass(t)
var a={cmd:"file-dialog-nw",args:[],handler:"",data:{dbg:!1,wDnD:wx.wDragAndDrop,wgtid:t,wPosX:wx.wPositionX+1,wPosY:wx.wPositionY+1}}
...
break
case BORDER_SIMPLE_LINE_WIDER:$tag.addClass("appx-border-simple-line-wider")}wx.wColorFgDisabled&&$tag.attr("style",$tag.attr("style")+";border-color:"+wx.wColorFgDisabled+" !important")}else $tag.is("fieldset")&&$tag.addClass("appx-border-etched-sunken")
...
var t={cmd:"dnd",args:[],handler:"",data:{dbg:!1,wDnD:wx.wDragAndDrop,wPrnt:WIDGET_PRNT_TYPE_WIDGET,wPosX:wx.wPositionX+1,wPosY:wx.wPositionY+1}}
...
return null===s&&null!==e.wCommandValueAdjusted&&(s=e.wCommandValueAdjusted),t=$("<button type='button'>").css({margin:"0",padding:"0"}),t.addClass("button").data("row",e.wPositionY).data("col",e.wPositionX),arguments.length>2&&null==e.wLabel&&null!=a.data&&(e.wLabel=a.data),null!=s&&(t.attr("id",addClientId("widget_sac_"+s,e.wClientId)),t.click(function(){var e=parseInt(getClientId(this.id).substring(11))
...
break
case"SPCB":this.wPcbId=parseInt(o)
break
...
break
case"TSTR":this.tableShowTableRefresh="T"==o
break
