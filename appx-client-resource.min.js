...
else try{img=new Image,img.src=r,appx_session.image_cache.hasOwnProperty(p)&&(appx_session.image_cache[p].url=r,img.onload=applyimage(p)),0==Math.abs(appx_session.current_show.curraction[0]&M_WAIT)&&0==appx_session.pendingResources.length&&sendappxshow(0,[])}catch(o){console.log("appx-client-resource.js AppxResource.handler() failed to load "+p+" image:"+o),console.log(o.stack),applyimage(null)}},AppxResource.load=function(e){var s=!0,a=e.split("."),p=a[3].split(","),n=appx_session.image_cache,t=parseInt("0x"+p[0]),r=cleanClassName(appx_session.server+"_"+appx_session.port+"_"+a[0].replace("#","")+"_"+a[1]+"_"+a[2]),o=r+a[5].split(",")[0]
