...
r.hasClass("appx-scroll-act")&&(r=$(".appxbox.appx-scroll").parent(),d=r.height()),x+17*t>d&&(t=(d-x)/17),a.appendTo(r).attr("size",t).blur(appxFireObjectEvent).click(appxFireObjectEvent).css({position:"absolute","z-index":c,"min-width":s.width()}).focus().offset(l),appx_session.objFocus=a}else appx_session.objFocus=$("<select>").val("0"),appxFireObjectEvent()}}catch(_){console.log("appxobjectinvokehandler: "+_),console.log(_.stack)}}function appxFireObjectEvent(){if(appx_session.objFocus&&!appx_session.objEventFired){console.log(appx_session.objEventFired),appx_session.objEventFired=!0
