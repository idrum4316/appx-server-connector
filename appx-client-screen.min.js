 ...
 *********************************************************************/
"use strict"
function getInputElement(e){return $(e).hasClass("appxdatefield")&&$(e).find(".appxdatevalue")&&(e=$(e).find(".appxdatevalue")),e}function setInputFocus(e){$(e).hasClass("appxdatefield")&&$(e).find(".appxdatevalue")&&(e=$(e).find(".appxdatevalue")),$(e).hasClass("appxcolorpickerwrapper")&&$(e).find(".appxcolorpicker")&&(e=$(e).find(".appxcolorpicker")),e.focus()}function appxmsgshandler(e){appx_session.msgs=[]
...
110==e.keyCode||78==e.keyCode?(a.data("checked",0),a.prop("indeterminate",!1),a.prop("checked",!1),a.val("N")):121!=e.keyCode&&89!=e.keyCode||(a.data("checked",2),a.prop("indeterminate",!1),a.prop("checked",!0),a.val("Y")),t.data("checked",a.data("checked")),t.prop("indeterminate",a.prop("indeterminate")),t.prop("checked",a.prop("checked")),t.val(a.val()),t.addClass("dirty"),a.val()!==s&&a.change()})}})}catch(a){console.log("appxApplyStylesCheckbox: "+a),console.log(a.stack)}}function appxApplyStylesColorPicker(){try{$.each($("#screenBuf .appxcolorpicker"),function(e,a){$(a).parent().width($(a).parent().width()+2*appx_session.colWidthPx-2)
var s=a.value.trim()
s.length>=6&&("#"!=s.substring(0,1)&&(s="#"+s),$(this).css({"border-color":s,"background-color":s})),$(a).change(function(){$(this).parent().find("img").colpickSetColor(this.value,this.value)
var e=this.value.trim()
e.length>=6&&("#"!=e.substring(0,1)&&(e="#"+e),$(this).css({"border-color":e,"background-color":e}))}),$(a).focus(function(){appx_session.getProp("autoSelect")&&$(this).select()}),$(a).parent().find("img").colpick({layout:"hex",submit:!0,colorScheme:"light",color:0==a.value.trim().length?"ffffff":a.value.trim(),onChange:function(e,a,s,t,n){$(t).parent().find("input").css({"border-color":"#"+a,"background-color":"#"+a}),n||($(t).parent().find("input").val("#"+a),$(t).parent().addClass("dirty"))},onSubmit:function(e,a,s,t,n){$(t).parent().find("input").css({"border-color":"#"+a,"background-color":"#"+a}),n||($(t).parent().find("input").val("#"+a),$(t).parent().addClass("dirty")),$(t).colpickHide()},onBeforeShow:function(e){var a=$(this).parent().find("input").val()
a=0==a.trim().length?"ffffff":a,$(this).colpickSetColor(a,a)},onShow:function(e){var a=$(this)
$(e).keydown(function(e){"Escape"!==e.key&&"F8"!==e.key||a.colpickHide(),"Enter"===e.key&&a.colpickHide(),e.stopPropagation()}),setTimeout(function(){var a=$("#"+e.id+" > .colpick_hex_field > input")
a.focus(),a.select()},1)}})})}catch(e){console.log("appxApplyStylesColorPicker: "+e),console.log(e.stack)}}function appxApplyStylesDate(){try{$.each($("#screenBuf .appxdatefield"),function(e,a){$(a).width($(a).width()+2.5*appx_session.colWidthPx)
...
var u=$('<button title="Launch a Full Featured HTML Editor">')
u.attr("id","CKE_Button"),u.val("a"),u.addClass("cke_fullscreen_button"),u.click(function(){var e=n,a=i,s=r
...
1==_.pos_row&&(g+=_.string.trim()+" ",_.isTitle&&(_.string=""))}g.length>0&&null==d.widget.wLabel&&(d.widget.wLabel=g)}var f=(d.end_column-d.begin_column)*o.w+3*o.w+2,w=(d.end_row-(d.begin_row-1))*o.h+1*o.h
if(appxIsScroll(d)?w-=1*o.h-4:w+=1*o.h,w>=document.documentElement.clientHeight&&d.data.indexOf("SBN=0")>-1&&"Firefox"===checkBrowser()&&(w+=appx_session.rowHeightPx),0==c)var b=w
var m=d.begin_row*o.h-o.h,v=d.begin_column*o.w-o.w
if(d.widget&&d.widget.wPositionX&&d.widget.wPositionY&&(m=d.widget.wPositionY*o.h-o.h,v=d.widget.wPositionX*o.w-o.w),!appxIsScroll(d)&&!appxIsScrollReg(d)){var y=($("#appx_main_container").height()-w)/2
if((0===c||y>appx_session.centerVerticalOffset)&&(appx_session.centerVerticalOffset=y),0==d.widget.specialHorizontalLocation&&(v=0),0==d.widget.specialVerticalLocation&&(m=0,c>0)){var S=d.end_row-d.begin_row,k=appx_session.current_show.boxes[c-1].end_row-appx_session.current_show.boxes[c-1].begin_row
S<=k&&(m=(b-w)/2)}}var C=$("#appx_main_container").width(),T=(C-f)/2
if(T<0&&(T=0),(0===c||T<appx_session.centerHorizontalOffset)&&appx_session.getProp("centerAppx")){var I=appx_session.centerHorizontalOffset
appx_session.centerHorizontalOffset=T
for(var h=0;h<c;h++){var P=appx_session.current_show.boxes[h],E=$("#box_"+P.widget.wBoxNumber.toString())
if(E.length){var O=E.position().left-I
O<0&&(O=0),E.css({left:O+appx_session.centerHorizontalOffset+"px"})}}}T>appx_session.centerHorizontalOffset&&!appxIsScroll(d)&&!appxIsScrollReg(d)&&0==d.widget.specialHorizontalLocation?T-=appx_session.centerHorizontalOffset:T=0,appxIsScroll(d)&&(v+=3,f-=6),appxIsScrollReg(d)&&(v+=3,f-=6)
var A=null
if(d.widget.wLabel&&""!=d.widget.wLabel){var R=2,B=R,D=R,L=f-4*R+"px",M=o.h+"px"
"true"==appxFillWindow&&d.end_column-d.begin_column+1==appx_session.screencols&&(L="100%",B="0px",D="0px")
var A=$("<div>").css({border:"0px","box-sizing":"border-box",display:"table",height:M,left:D,overflow:"hidden",position:"absolute",top:B,width:L,"z-index":x+1})
A.addClass("appx-title"),A.attr("id","box_2_"+d.widget.wBoxNumber.toString()),A.data("row",d.widget.wPositionY),A.data("col",d.widget.wPositionX),d.widget.isBox=!1,appxwidgetshandlerpropsex(d.widget,A,!0)}v=v+T+appx_session.centerHorizontalOffset+"px","true"==appxFillWindow&&1==d.begin_row&&1==d.begin_column&&d.end_row==appx_session.screenrows-3&&d.end_column==appx_session.screencols&&(f="100%")
var N=$("<div>").css({"box-sizing":"border-box",height:w,left:v,overflow:"hidden",position:"absolute",top:m,width:f,"z-index":x})
if(A&&N.append(A),N.addClass("appxbox"),N.attr("id","box_"+d.widget.wBoxNumber.toString()),N.data("row",d.widget.wPositionY),N.data("col",d.widget.wPositionX),N.attr("data-role","content"),1==d.begin_row&&1==d.begin_column&&d.end_row==appx_session.screenrows-3&&d.end_column==appx_session.screencols&&N.addClass("appxbox-full"),d.widget.isBox=!0,appxwidgetshandlerprops(d.widget,N),$_screenBuf.append(N),p.row>=d.begin_row&&p.row<=d.end_row&&p.col>=d.begin_column&&p.col<=d.end_column&&(null==_boxcur||_boxcur.end_row-_boxcur.begin_row>=d.end_row-d.begin_row||_boxcur.end_column-_boxcur.begin_column>=d.end_column-d.begin_column)&&0==Math.abs(d.bit_mask&SCROLL)&&(_boxcur=d,n=N),!(d.end_column-d.begin_column+1>=appx_session.screencols&&d.end_row-d.begin_row+1>=appx_session.screenrows-3))if(appxIsScroll(d))N.addClass("appx-scroll"),N.css("top",m+o.h)
...
for(var a=0;a<appx_session.rowtext.length;a++){var _=1,f=appx_session.rowtext[a]
if(f.type==ROWTEXT_TYPE_ITEM)s=appxitembox(f.pos_row,f.pos_col,f.size_rows,f.size_cols)
else{var w=f.boxid,b=e[w]
if(b>=0){var s=appx_session.current_show.boxes[b]
if(!appxIsFullscreen(s)&&appxIsScroll(s)){var x=s
s=appxitembox(x.begin_row+f.pos_row-1,x.begin_column+f.pos_col-1,f.size_rows,f.size_cols),_=x.begin_row}}}appxIsScroll(s)||appxIsScrollReg(s)?(f.pos_row-=s.begin_row-_,f.pos_col-=s.begin_column-1):f.type==ROWTEXT_TYPE_ITEM&&(f.pos_row-=s.begin_row-1,f.pos_col-=s.begin_column-1),s.rowtext.push(f)}appx_session.rowtext=[]}catch(t){console.log("appxnestwidgets: "+t),console.log(t.stack)}}function appxshowhandler(e){try{if(null==e||null==e.data)return
...
for(var _=0;_<e.items.length;_++){var f=e.items[_]
if(4===f.length&&f[3]===r){e.items[_][1]=h
break}}if(null!=e){var w=$("#screenBuf #box_"+i)
w.length&&w.append(h)}}}}function appxshowitemsforbox(e,a,s){for(var t=e.items.length-1;t>=0;t--){var n=e.items[t][0],o=null
...
var _=parseInt(i.css("width"))-4,f=_<x
if(h||f){appxwidgetdimensions(o,h,e)
var w=_,b=!0
if(f||(w=parseInt(h.css("left")),b=g===parseInt(h.css("top"))),b&&x>w){var m=r.attr("data-padWidth"),v=x-m,y=4*appx_session.colWidthPx
if(v<w){var S=f?5:1,k=w-S-u
k>y?r.width(k+"px"):r.width(y.toString()+"px")}}}}var C=!1
if(n.type==ROWTEXT_TYPE_ITEM){var T=$("<div>"),I=$("<div>")
null!=n.widget.wSepBefore&&n.widget.wSepBefore===!0&&(C=!0,T.addClass("sepBefore"),i.append(T),r.removeClass("sepBefore")),null!=n.widget.wSepAfter&&n.widget.wSepAfter===!0&&(C=!0,I.addClass("sepAfter"),i.append(I),r.removeClass("sepAfter"))
var P={}
$.each(r[0].attributes,function(e,a){P[a.nodeName]=a.nodeValue})
for(var E in P)if("style"==E){var O=10
null===n.widget.wLayer&&void 0===n.widget.wLayer||(O=n.widget.wLayer),n.widget.wSepBefore===!0&&(T.attr(E,P[E]),T.css({height:parseInt(s.css("height"))+"px"}),T.css({left:parseInt(r.css("left"))-4+"px"}),T.css({width:"1px"}),T.css({top:"0px"}),T.css({"z-index":e.layer+O*-1})),n.widget.wSepAfter===!0&&(I.attr(E,P[E]),I.css({height:parseInt(s.css("height"))+"px"}),I.css({left:parseInt(r.css("left"))+parseInt(r.css("width"))-2+"px"}),I.css({width:"1px"}),I.css({top:"0px"}),I.css({"z-index":e.layer+O*-1}))
break}}if(e.items[t].length<4){if(void 0===n.widget||null==n.widget||void 0===n.widget.wWidgetType||null==n.widget.wWidgetType||n.widget.wWidgetType!=WIDGET_TYPE_CHECK_BOX&&n.widget.wWidgetType!=WIDGET_TYPE_NONE&&n.widget.wWidgetType!=WIDGET_TYPE_RAW_TEXT&&n.widget.wWidgetType!=WIDGET_TYPE_LABEL||n.widget.wSepBefore!==!0&&n.widget.wSepAfter!==!0||0!=C)i.append(r)
else{var T=$("<div>"),I=$("<div>")
n.widget.wSepBefore===!0&&(T.addClass("sepBefore"),r.removeClass("sepBefore"),i.append(T)),n.widget.wSepAfter===!0&&(I.addClass("sepAfter"),r.removeClass("sepAfter"),i.append(I)),i.append(r)
var P={}
$.each(r[0].attributes,function(e,a){P[a.nodeName]=a.nodeValue})
for(var E in P)if("style"==E){var O=10
null===n.widget.wLayer&&void 0===n.widget.wLayer||(O=n.widget.wLayer),n.widget.wSepBefore===!0&&(T.attr(E,P[E]),T.css({height:parseInt(s.css("height"))+"px"}),T.css({left:parseInt(r.css("left"))-4+"px"}),T.css({width:"1px"}),T.css({top:"0px"}),T.css("z-index",e.layer+O*-1)),n.widget.wSepAfter===!0&&(I.attr(E,P[E]),I.css({height:parseInt(s.css("height"))+"px"}),I.css({left:parseInt(r.css("left"))+parseInt(r.css("width"))-2+"px"}),I.css({width:"1px"}),I.css({top:"0px"}),I.css({"z-index":e.layer+O*-1}))
break}}var O=10
null===n.widget.wLayer&&void 0===n.widget.wLayer||(O=n.widget.wLayer),r.css("z-index",e.layer+O*-1)}if(appxIsScannable(n)){var A=appxGetCellSize(),R=1,B=n.pos_col*A.w,u=parseInt(r.css("left")),D=B-u,L=u+parseInt(r.css("width"))+R,M=L+9,N=parseInt(r.css("top"))+4,U=!0
if(o){var w=o.pos_col*A.w-D
n.pos_row*A.h
n.pos_row==o.pos_row&&(L>=w||M>=w)&&(U=!1)}if(U){var W=n.widget.wPositionX,z=n.widget.wPositionY
...
for(var x=0;x<o.length;x++)for(var i in t[p])if(t[p][i].tabGrp===o[x])for(var f=0;f<t[p][i].length;f++)a(t[p][i][f].tag,t[p][i].tabGrp)}}appx_session.tablist=e}}catch(w){console.log("appxSetTabIndexes: "+w),console.log(w.stack)}}function sendappxshow(e,a){appx_session.buildingEm||clearAndReset(),appx_session.showCursor=!1,appxSetStatusStateText(APPX_STATE_BUSY)
...
1==g&&(w="$(sendFile)\\"+f)
var b,m
b=$(a[u]).data("row"),m=$(a[u]).data("col"),appx_session.dndData.push({row:b,col:m,parentType:$(a[u]).data("parent_type"),path:h+f,name:w,ext:f.substring(f.lastIndexOf(".")+1).toLowerCase(),mtype:_,size:appx_session.filesUploadArray[l].size.toString(),props:[]})}}else x=o.indexOf("\\")>-1||o.indexOf("/")>-1?o.indexOf("\\")>-1?"$(sendFile)"+o.substring(o.lastIndexOf("\\")):"$(sendFile)"+o.substring(o.lastIndexOf("/")):"$(sendFile)\\"+o}else x=$(a[u]).val()
...
d.push(S)}var k=toUTF8Array(x)
if((appx_session.server_extended_feature_mask&TMNET_FEATURE2_LARGE_WORK_FIELD)==TMNET_FEATURE2_LARGE_WORK_FIELD)Array.prototype.push.apply(d,hton32(k.length))
else{var C=k.length%256,T=(k.length-C)/256
d.push(T),d.push(C)}for(l=0;l<k.length;l++)d.push(k[l])}clearClientIds(),$(a[0]).hasClass("DnD")?(c={cmd:"appxmessage",args:[0,0,0,0],handler:"appxshowhandler",data:null},appx_session.ws.send(JSON.stringify(c))):(c={cmd:"appxmessage",args:hton32(a.length),handler:"appxshowhandler",data:null},appx_session.ws.send(JSON.stringify(c)),c={cmd:"appxmessage",args:d,handler:"appxshowhandler",data:null},appx_session.ws.send(JSON.stringify(c)))}else c={cmd:"appxmessage",args:[0,0,0,2,86,0,0,0,0,0,0,0],handler:"appxshowhandler",data:null},appx_session.ws.send(JSON.stringify(c))
...
if(P.selkeys)for(var E=0;E<P.selkeys.length;E++){var x=""
x=P.selkeys[E]
...
var O=hton32(I+1)
Array.prototype.push.apply(d,O),d.push(6),d.push(x.length/2)
for(var A=0;A<x.length;A+=2)d.push(parseInt(x.substring(A,A+2),16))
I++}}var R=hton32(I)
